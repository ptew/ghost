...
def test_stuff():
  ...
  for p in pdb.query(zoobar.zoodb.Person).all():
    if p.zoobars < 10:
      #If their balance has gone down, check if they issued the requests
      transfers = tdb.query(zoobar.zoodb.Transfer).filter(zoobar.zoodb.Transfer.sender == p.username)
