...
def concolic_test(testfunc, maxiter = 100, verbose = 0):
  ...
  while iter < maxiter and not inputs.empty():
    ...
    for branch_index in range(len(cur_path_constr)):
      #print branch_index
      #print len(cur_path_constr)
      #print cur_path_constr
      #print checked
      ##
      ...
      #print inputs
      #print
      #for item in cur_path_constr[0:branch_index]:
      #  print item
      
      #if one item long, do opposite of that branch
      if branch_index == 0:
        opp_branch = sym_not(cur_path_constr[0])
      else:
        #start with first good item
        opp_branch = cur_path_constr[0]
        
        #from the 2nd item to the 2nd to last
        for item in cur_path_constr[1:branch_index]:
          #add them
          opp_branch = sym_and(opp_branch, item)
        #add NOT the last item
        opp_branch = sym_and(opp_branch, sym_not(cur_path_constr[branch_index]))
       
      if (opp_branch in checked):
        pass
      else:
        ##   it (otherwise, add it to prevent further duplicates). 
        checked.add(opp_branch)
      
        ## - Invoke Z3, along the lines of:
        ##
        ##     (ok, model) = fork_and_check(constr)
        ##
        (ok, model) = fork_and_check(opp_branch)
        ...
        if ok == z3.sat:
          new_values = {}
          for key in concrete_values:
            if key in model:
              new_values[key] = model[key]
            else:
              new_values[key] = concrete_values[key]
          caller = cur_path_constr_callers[branch_index]
